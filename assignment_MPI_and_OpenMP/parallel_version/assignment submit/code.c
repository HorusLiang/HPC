...
int main(){   
    ...
    //Compute rows per processes
    int rows_per_proc = (int)ceil((double)ROW / num_procs);
    int row_size = (rows_per_proc+2) * ROW;

    int *A = NULL; //A is global board
    int *C = NULL; //C and D is local board for each process
    int *D = NULL;
    ...
    // Error handling for malloc: if an error occurs with one of the memory allocations, stop running.
